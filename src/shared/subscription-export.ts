...
import type { SubscriptionClientId, SubscriptionKind } from "./subscription-clients.js";
...
export function convertQuantumProxy(proxy: ProxyRecord, name: string): ProxyConversion {
...
export function convertSurgeProxy(proxy: ProxyRecord, name: string): ProxyConversion {
...
export function convertLoonProxy(proxy: ProxyRecord, name: string): ProxyConversion {
  const type = textValue(proxy, "type").toLowerCase();
  switch (type) {
    case "ss":
    case "shadowsocks":
...
function buildProxyDocument(
  ...
  const filenames = {
    surge: "surge-nodes.conf",
    "quantumult-x": "quantumult-x-nodes.conf",
    loon: "loon-nodes.conf",
  } as const;
  const comments = [
...
function buildShadowrocketDocument(proxies: ParsedProxy[]): SubscriptionDocument {
  ...
  return {
    ...
    contentType: "text/plain; charset=utf-8",
    filename: "shadowrocket-nodes.txt",
    proxyCount: lines.length,
    skippedCount: warnings.length,
    warnings,
  };
